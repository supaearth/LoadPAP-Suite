...
- Repo-local override: do not automatically push this repository to GitHub, even if global/workspace auto-sync rules would otherwise apply.
- Only run `git push` after the user explicitly asks to push/sync/publish in the current conversation.
- At task end, leave local changes unpushed and report changed files/status instead.
...
## Known Bugs / Current Debt

- Resolved/stale note: PyRUSH no longer locally redefines `extract_id()` / `get_folder_path()` in the active V3 page; it imports shared helpers from `utils.py`.
- Resolved/stale note: PyLOAD active V3 retry/social download path passes both video and image directories to `download_worker()`.
- Current debt: dynamic HTML rendering remains broad across Streamlit pages. Use `utils.escape_html()` and `utils.js_literal()` for new or touched unsafe HTML/JS paths.
- Current debt: Cloud Run DEV deployment exists for smoke/browser testing, but macOS local Streamlit remains the canonical production distribution model.
